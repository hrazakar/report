...
public class mainReport {
    ...
    private static boolean INTERPOLATION; //Distance en kilomètre
	...
	public static String createstopsInfos(Connection c, Statement s, String dvId, int irang,
		...
		for (int i = 0; i < pList.size(); i++) {
			...
			if (evt.getDeviceID().equals(dvId)) {
				...
				{
					pListemp.add(new Events(evt.getAccountID(), evt.DeviceID, evt.getLatitude(), evt.getLongitude(), evt.getSpeed(), evt.getDateEvt(), evt.getOdometerKM(), evt.getOdometerOffsetKM(),(int) evt.getStatusCode(),(int) evt.getSpeedMax(), evt.getHeading()));					
				}
		...
		DecimalFormat dec = new DecimalFormat("#00.000000", formatSymbols);
  		for (int i = 0; i < pList.size();i++){
  			...
  			if (evt.getDeviceID().equals(dvId)){
  				...
  				pathinfos = pathinfos + "Array("+dec.format(evt.getLatitude())+", " + dec.format(evt.getLongitude()) +", " + datem +", " + evt.getHeading() + ", "+ (int) evt.getSpeed()+")";
  				nbpath++;
	        ...
	        for (int i=0; i < pList.size() ; i++){
       			...
       			if (evt.getDeviceID().equalsIgnoreCase(dvId)){   
	 	            ...
	 	            if (evt.getSpeed() > evt.getSpeedMax()){ // Vérification vitesse maximum
	 			        ...
	 			        eventsInfos = eventsInfos + "Array("+dec.format(evt.getLatitude())+", " + dec.format(evt.getLongitude())+", " + new SimpleDateFormat("yyyyMMddHHmmss").format(evt.getDateEvt()) +", " + new SimpleDateFormat("HHmmss").format(evt.getDateEvt()) +",\"\", \"Vitesse maximale\", " + evt.getSpeed() +", "+limit+")";
	 			        nbevents++;	 		            	
	 	            }
 	            }
	        }
	...
	public static LinkedList createTemporaryTable(Connection c, Statement s, String account) {
        ...
        try {
            ...
            if (rs1.next()) {
            	...
            	sql = "SELECT accountID, FROM_UNIXTIME(timestamp) as timestamp, deviceID, latitude, longitude, speedKPH, odometerKM, odometerOffsetKM, statusCode, heading FROM EventData WHERE accountID='"+account+"' and longitude <> 0.0 and FROM_UNIXTIME(timestamp) >= '"+date_trt+"' and FROM_UNIXTIME(timestamp) <= '"+date_fin+"' order by deviceID,timestamp;";
	            ...
	            if (rs.next()) {
                	...
                	eventsList.add(new Events(acIDlast, devIDlast, latitudelast, longitudelast, vitesselast, dateEVlast, odometerKMlast, odometerOffsetKMlast, statusCode, speedLimit, headinglast));
                	nbevt++;
	                	...
	                	if (devIDlast.equalsIgnoreCase(devID)){
		                	/*if (vitesse == 0.0 && vitesselast == 0.0 && (odometerKM - odometerKMlast) < 1){
		                		...
		                		if (INTERPOLATION){
			                		...
			                		duree = (int) diff / 1000;
			                		if (duree > 120){
			                			eventsList = addpointsList(acID, devID, eventsList, latitude, longitude, dateEV, vitesse, odometerKM, odometerOffsetKM, statusCode, speedLimit,heading);
			                			nbevt++;
			                		} else {
			                			eventsList.add(new Events(acID, devID, latitude, longitude, vitesse, dateEV, odometerKM, odometerOffsetKM, statusCode, speedLimit, heading));
			                			nbevt++;
    		...
    		for (int j = 0; j < eventsList.size(); j++) {
                ...
                System.out.println(evt.getAccountID() + " " + evt.getDeviceID() + " " + new SimpleDateFormat("yyy-MM-dd HH:mm:ss").format(evt.getDateEvt()) + " " + dec.format(evt.getLatitude()) + " "+ dec.format(evt.getLongitude()) + " " + evt.getSpeed() + " " + evt.getOdometerKM() + " " + evt.getSpeedMax() + " "+evt.getStatusCode() + " "+ evt.getHeading());
     			 ...
     			 if (nbrpoints > 1){
	     			 ...
	     			 if (polyline != null){
	    				...
	    				for (int ip = 0; ip < polyline.size(); ip++) {
	    				    ...
	    				    pListnw.add(new Events(acId, dvId, lat, lon, vitessenv, tm , 0.0, 0.0, -1, (int)evt.getSpeedMax(),0));	
	...
	public static LinkedList addpointsList(String acId, String dvId, LinkedList pList, double lat, double lon, Timestamp dateEV,
			double vitesse, double odometerKM, double odometerOffsetKM, int statusCode, int speedLimit, int heading){
		double distance;
    			...
    			if (polyline != null){
    				...
    				//System.out.println("Nombre de point nv:" + polyline.size()+ " tmps a ajouter = "+ tempsnv+ " dif:"+temps);
	   				for (int ip = 0; ip < polyline.size(); ip++) {
		     			...
		     			//System.out.println("ip:" + ip+ "tmps = "+ tmps + " vitessenv: "+vitessenv);
	   				    //System.out.println("donnée à l'indice " + ip + " = " +lat+ "," +lon+", "+tm.getTime()+" vit: "+vitessenv);
		     			pList.add(new Events(acId, dvId, la, lo, (int) vitessenv, tm , evt.getOdometerKM()+(dist*(ip+1)), 0.0, -1, speedLimit,heading));	
   				...
   				//pList.addAll(pList.size() - 1, pListnw);
    				...
    				pList.add(new Events(acId, dvId, lat, lon, (int) vitesse, dateEV , odometerKM, odometerOffsetKM, statusCode, speedLimit,heading));
	...
	private static void readConfig(String file) {
		...
		try {
			...
			if (email.equalsIgnoreCase("true")) SEND_EMAIL = true;
			else SEND_EMAIL = false;
			String interpol = prop.getProperty("interpolation");
			if (interpol.equalsIgnoreCase("true")) INTERPOLATION = true;
